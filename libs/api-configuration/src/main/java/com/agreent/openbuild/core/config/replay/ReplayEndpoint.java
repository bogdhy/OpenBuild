...
public class ReplayEndpoint {
  ...
  private final ConcurrentHashMap<
    ReplayableEventProcessor, ReplayableEventProcessor.ListenerRegistry>
    replayingProcessors;
  ...
  public void startReplay(
    @Nullable Set<String> processingGroups, @Nullable OffsetDateTime startTime) {
    var replayableEventProcessors =
    ...
    synchronized (this) {
      ...
      replayableEventProcessors.forEach(
        x -> {
          replayingProcessors.put(
            x, x.registerReplayCompletionListener(this::onSingleProcessorReplayCompletion));
  ...
  private void onSingleProcessorReplayCompletion(ReplayableEventProcessor processor) {
    synchronized (this) {
      ...
      if (listenerRegistry == null) {
        log.warn("Processor not registered for replaying: {}", processor);
        return;
      }
      try {
        listenerRegistry.close();
      } catch (IOException e) {
        log.error("Cannot de-register listener for processor: {}", processor, e);
      }
      if (replayingProcessors.size() == 0) {
        log.info("Executing reset completion tasks");
        taskExecutor.execute(
          () ->
            replayCompletionAwareListeners.forEach(
              t -> {
                try {
                  t.replayCompleted();
                } catch (Exception e) {
                  log.error("Error when running replay completion aware task", e);
                }
              }));
      }
    }
