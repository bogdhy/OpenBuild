package com.agreent.openbuild.core.config.replay;
...
@Slf4j
public class ReplayMarkerAwareTrackingEventProcessor extends TrackingEventProcessor
  implements ReplayableEventProcessor {

  private final AtomicReference<ReplayMarkerEvent> targetMarkerEventHolder =
    new AtomicReference<>();
  private final AtomicInteger workerReplayCompletionCounter = new AtomicInteger();
  private final List<Consumer<ReplayableEventProcessor>> replayCompletionListener =
    new ArrayList<>();
  ...
  @Override
  public ListenerRegistry registerReplayCompletionListener(
  ...
  @Override
  protected boolean canHandle(EventMessage<?> eventMessage, Collection<Segment> segments)
    throws Exception {
  ...
  private void processReplayMarkerEvent(EventMessage<?> eventMessage) {
    log.info("Processing target replay marker event: {}", eventMessage.getPayload());
    int numberOfActiveSegments = processingStatus().size();
    if (workerReplayCompletionCounter.incrementAndGet() == numberOfActiveSegments) {
      synchronized (this) {
        log.info("Replay completed: {}", numberOfActiveSegments);
        targetMarkerEventHolder.set(null);
        taskExecutor.execute(
          () ->
            List.copyOf(replayCompletionListener)
              .forEach(
                l -> {
                  try {
                    l.accept(this);
                  } catch (Exception e) {
                    log.error("Error running replay completion listener", e);
                  }
                }));
      }
    }
