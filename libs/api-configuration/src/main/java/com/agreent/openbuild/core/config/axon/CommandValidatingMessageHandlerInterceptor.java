...
import com.agreent.openbuild.core.validation.ValidatableCommand;
import com.agreent.openbuild.core.validation.Validates;
...
@Component
public class CommandValidatingMessageHandlerInterceptor
  ...
  public CommandValidatingMessageHandlerInterceptor(
    List<Validates> validators, Validator javaBeanValidator) {
    this.javaBeanValidator = javaBeanValidator;
    Map<Class<?>, Validates<?>> m = new ConcurrentHashMap<>();
    for (Validates<?> validator : validators) {
      Type validatableCommandType =
        Arrays.stream(validator.getClass().getGenericInterfaces())
  ...
  private <T extends ValidatableCommand> void validate(CommandMessage<?> commandMessage) {
    ...
    List<Class<?>> validatableInterfaces =
      getValidatableInterfaces(commandMessage.getPayloadType());
  ...
  private List<Class<?>> getValidatableInterfaces(Class<?> commandClass) {
    List<Class<?>> interfaces =
      Arrays.stream(commandClass.getInterfaces())
